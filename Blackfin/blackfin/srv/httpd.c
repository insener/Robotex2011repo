...
void httpd_request(char firstChar)
{
    ...
    if (reqBuf[0] == 'G') {
        ...
        if (strncmp(path, robotJpg, countof (robotJpg) - 1) == 0) {
            ...
            imgHeight = frame.imageHeight;
            imgWidth = frame.imageWidth;
            contentLength = jpeg_encodeImage((unsigned char *)FRAME_BUF, (unsigned char *) body, FOUR_TWO_TWO, imgWidth, imgHeight) - (unsigned char *) body;
            ...
            switch (params[0]) {
                ...
                case 'q':  // set JPEG quality 1 - 8 (1 is highest)
                    if ((params[1]<'1') || (params[1]>'8'))  // command out of range ?
                        break;
                    jpeg_setQualityFactor(params[1] & 0x0F);
                    break; 
                case 'i': 
                    switch (params[1]) {
                ...
                case 'v':  // process colors
                    ...
                    switch (params[1]) {
                        case 'a':  //    va = enable/disable AGC(4) / AWB(2) / AEC(1) camera controls
                            ...
                            i2c_data[0] = 0x13;
                            i2c_data[1] = 0xC0 + ix;
                            i2c_write(0x30, (unsigned char *)i2c_data, 1, SCCB_ON);  // OV9655
                            i2c_write(0x21, (unsigned char *)i2c_data, 1, SCCB_ON);  // OV7725
                            break;
