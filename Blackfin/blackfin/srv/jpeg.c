...
void initialization (JPEG_ENCODER_STRUCTURE *jpeg, UINT32 image_format, UINT32 image_width, UINT32 image_height)
{
    ...
    _qualityFactor = 1; // default
}

/*
 * Encodes a image in jpeg format
 */
UINT8* jpeg_encodeImage (UINT8 *input_ptr,UINT8 *output_ptr, UINT32 image_format, UINT32 image_width, UINT32 image_height)
{
    UINT16 i, j;
...
/*
 * Set the quality factor of jpeg image. The range is between 1-8.
 * 1 = highest quality,  8 = lowest quality. The default value is 4.
 */
void jpeg_setQualityFactor(unsigned int quality)
{
	if (quality <= 1)
	{
		_qualityFactor = 1;
...
void initializeQuantizationTables ()
{
    ...
    if (_qualityFactor > 8)
    	_qualityFactor = 8;
    _qualityFactor = ((_qualityFactor * 3) - 2) * 128; //converts range[1:8] to [1:22]
    ...
    for (i=0; i<64; i++)
    {
        ...
        value = luminance_quant_table [i] * _qualityFactor;
        ...
        value = chrominance_quant_table [i] * _qualityFactor;
        value = (value + 0x200) >> 10;
